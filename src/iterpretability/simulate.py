...
class SyntheticSimulator:
    ...
    def simulate_dataset(
        ...
        if self.treatment_assign == "top_prog":
            po0 = self.scale_factor * ((prog + self.addvar_scale * X[:, top_idx])
                                       + self.predictive_scale * pred0
                                       )
            po1 = self.scale_factor * ((prog + self.addvar_scale * X[:, top_idx])
                                       + self.predictive_scale * pred1
                                       )
        elif self.treatment_assign == "top_pred":
            po0 = self.scale_factor * (prog + self.predictive_scale * pred0)
            po1 = self.scale_factor * (
                prog + self.predictive_scale * pred1 + self.addvar_scale * X[:, top_idx]
            )
        else:
            po0 = self.scale_factor * (prog + self.predictive_scale * pred0)
    ...
    def po0(self, X: torch.Tensor) -> torch.Tensor:
        ...
        if self.treatment_assign == "top_prog":
            po0 = self.scale_factor * ((prog_factor + self.addvar_scale * X[:, self.top_idx])
    ...
    def po1(self, X: torch.Tensor) -> torch.Tensor:
        ...
        if self.treatment_assign == "top_pred":
            po1 = self.scale_factor * (prog_factor
                                       + self.predictive_scale * pred1_factor
                                       + self.addvar_scale * X[:, self.top_idx]
                                       )
        elif self.treatment_assign == "top_prog":
            po1 = self.scale_factor * ((prog_factor + self.addvar_scale * X[:, self.top_idx])
                                       + self.predictive_scale * pred1_factor
                                       )
        else:
