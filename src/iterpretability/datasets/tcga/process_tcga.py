...
def process_tcga(max_num_genes):
    ...
    tcga_dataset['rnaseq'] = tcga_dataset['rnaseq'][:, np.where(
        np.min(tcga_dataset['rnaseq'], axis=0) - np.max(tcga_dataset['rnaseq'], axis=0) != 0)[0]]

    # Select max_num_genes
    filtered_gene_data = filter_genes(
        pd.DataFrame(tcga_dataset['rnaseq'], columns=range(tcga_dataset['rnaseq'].shape[1])), num_genes=max_num_genes,
