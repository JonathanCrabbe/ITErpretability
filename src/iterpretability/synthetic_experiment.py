...
class PredictiveSensitivity:
    ...
    def __init__(
        ...
        predictive_scales: list = [1e-3, 1e-2, 1e-1, 1, 2],
    ...
    def run(
        ...
        for predictive_scale in self.predictive_scales:
            ...
            learners = {
               ...
               #     n_iter=self.n_iter,
                ...
                "SNet_0.1": cate_models.torch.SNet(
                    X_train.shape[1],
                    binary_y=(len(np.unique(Y_train)) == 2),
                    n_layers_r=1,
                    n_layers_out=1,
                ...
                "SNet_0.001": cate_models.torch.SNet(
                    X_train.shape[1],
                    binary_y=(len(np.unique(Y_train)) == 2),
                    n_layers_r=1,
                    n_layers_out=1,
        ...
        metrics_df.to_csv(results_path / f"snetfro_predictive_scale_{dataset}"
                                         f"_{num_important_features}_"
...
class PropensitySensitivity:
    ...
    def run(
        ...
        results_path = self.save_path / "results/propensity_sensitivity"
        log.info(f"Saving results in {results_path}...")
        if not results_path.exists():
            results_path.mkdir(parents=True, exist_ok=True)
